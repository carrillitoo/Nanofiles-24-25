...
public class NFController {
	...
	private static final byte SERVING = 8;
	...
	private boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_QUIT: 
			if(currentState == OFFLINE || currentState == ONLINE){
				commandAllowed = true;
				break;
			}
		case NFCommands.COM_FILELIST: 
			if(currentState == ONLINE || currentState == SERVING) {
				commandAllowed = true;
				break;
			}
		case NFCommands.COM_UPLOAD: 
	...
	private void updateCurrentState(boolean success) {
	    ...
	    switch (currentCommand) {
	        case NFCommands.COM_PING:
	            if (currentState == OFFLINE ) {
	                currentState = ONLINE; // Si el ping es exitoso, pasamos a ONLINE
	        ...
	        default:
	            // No cambiamos el estado para comandos no reconocidos o que no afectan el estado
	            break;
	    }
	}
